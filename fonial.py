...
class Fonial(object):
    ...
    def  verify_user(self, user):
        ...
        if len(errors) == 1 and errors[0] == "outbound_num":
            if not user.fonial_number.state:
                logging.info("{}: outbound number {} inactive".format(user.name, user.fonial_number.number))
            else:
                logging.info("{}: fixing outbound number".format(user.name, user.fonial_number))
                response = self.session.post("https://kundenkonto.fonial.de/system/device/{}/update".format(user.fonial_device.id), data={
                    "fonial_frontend_device_ip_device[targetName]": user.name,  # <Name>
                    "fonial_frontend_device_ip_device[mac]": mac_address,  # <Mac-Address>
                    "fonial_frontend_device_ip_device[provisioning_template]": "",
                    "fonial_frontend_device_ip_device[keyextensions]": "",
                    "fonial_frontend_device_ip_device[outboundnum]": user.fonial_number.id,  # the ID of the fonial phone number
                    "fonial_frontend_device_ip_device[moh]": "4",
                    "fonial_frontend_device_ip_device[internalext]": user.extension,  # the 3-digit extension 678
                    "fonial_frontend_device_ip_device[automatic]": "1",
                    "fonial_frontend_device_ip_device[hash]": "ip",
                    "fonial_frontend_device_ip_device[targetType]": "IPDEVICE",
                    "fonial_frontend_device_ip_device[account]": self.args.account,   # <fonial account id>
    ...
    def set_phone_number(self, n, d):
        if not n.state:
            logging.warning("can not set an inactive number {}".format(n))
            return

        logging.info("setting phone number {} for device {}".format(n.number, d))
    ...
    def switch_number(self, source, destination, csrf=None):
        ...
        if response.text != "{}":
            print("FAILED SWITCHING {} -> {}".format(source, destination))
